...
async function forgotPassword(req, res, next) {
  ...
  if (!user) {
    return next(
      res.json({
        success: false,
        message: "The user doesn't exist please create account",
      })
    );
  } else {
...
async function resetPassword(req, res, next) {
  ...
  if (!user) {
    return next(
      res.json({
        success: false,
        message: "The time has been expired or token is invalid",
      })
    );
  } else {
...
async function updatePassword(req, res, next) {
  ...
  if (!user.comparePassword(req.body.oldPassword)) {
    return next(
      res.json({ message: "The passwords did not match", success: false })
    );
  }
  if (req.body.newPassword !== req.body.confirmPassword) {
    return next(
      res.json({
        success: false,
        message: "The new password and the confirm password did not match",
      })
    );
  }
  if (req.body.newPassword.length < 8) {
    return next(
      res.json({
        success: false,
        message: "The password legth should be 8 characters long",
      })
    );
  }
...
async function sendRequest(req, res, next) {
  try {
    ...
    const idx = order.team.findIndex(
...
async function acceptRequest(req, res, next) {
  try {
    ...
    const currTeamSize = order.team.filter(
    ...
    const userIndex = order.team.findIndex(
      (member) => member.user._id == req.body.userId
    );
    if (userIndex !== -1) {
...
const rejectRequest = async (req, res, next) => {
  try {
    ...
    user.myEvents = user.myEvents.filter(
      (event) => event.toString() !== order._id.toString()
    );
