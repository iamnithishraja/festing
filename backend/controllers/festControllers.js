...
async function updateFest(req, res, next) {
  try {
    ...
    if (req.files != null) {
      ...
      req.body.poster = await uploadImage(
        req.files.poster.data,
        fest.poster.public_id.split("/")[0]
      );
    }
...
async function createEvent(req, res, next) {
  try {
    ...
    const poster = await uploadImage(
      req.files.poster.data,
      fest.poster.public_id.split("/")[0] + "/" + req.body.name
    );
    const schedule = [];
    for (let i = 0; req.body[`schedule[${i}]`] != undefined; i++) {
      schedule.push([
        new Date(req.body[`schedule[${i}]`][0]),
        new Date(req.body[`schedule[${i}]`][1]),
      ]);
    }
...
async function updateEvent(req, res, next) {
  try {
    ...
    if (req.files != null) {
      ...
      req.body.poster = await uploadImage(
        req.files.poster.data,
        event.poster.public_id.split("/")[0]
      );
    }
    const schedule = [];
    for (let i = 0; req.body[`schedule[${i}]`] != undefined; i++) {
    ...
    const new_event = await Event.findByIdAndUpdate(req.body.eventId, {
      ...req.body,
      schedule: schedule,
    });
